...
final class ProviderPresentationSectionsTests: XCTestCase {
    ...
    func testBDD_GivenOriginalAndIntegratedAccountTemplates_WhenBuildingAccountProviders_ThenOnlyOriginalAccountProvidersAreReturned() {
        let providers = [
            Provider(
                id: "codex",
                kind: .vendor,
        ...
        XCTAssertEqual(sections.first?.providers.map(\.id), ["codex"])
    }
}
