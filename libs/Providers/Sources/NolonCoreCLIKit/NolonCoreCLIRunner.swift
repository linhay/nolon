...
public struct NolonCoreCLIRunner: Sendable {
    ...
    private func resolveGeminiAuthProvider(_ providerID: String) throws -> UsageProvider {
    ...
    private func geminiAuthStore() -> GeminiAuthStore {
        GeminiAuthStore.shared
    }

    private func parseGeminiAuthMethod(_ raw: String) throws -> GeminiAuthMethod {
    ...
    private func activeGeminiAccountID(provider: UsageProvider) async throws -> UUID? {
        try await geminiAuthStore().activeAccount(provider: provider)?.id
    }

    private func loadGeminiAccountListing(provider: UsageProvider) async throws -> (activeAccountID: UUID?, accounts: [GeminiAuthAccountView]) {
    ...
    private func geminiAuthActivate(provider: UsageProvider, accountID: String) async throws -> GeminiAuthMutationPayload {
    ...
    private func geminiAuthDelete(provider: UsageProvider, accountID: String) async throws -> GeminiAuthMutationPayload {
        guard let targetID = UUID(uuidString: accountID) else {
            throw NolonCoreCLIError.invalidArguments("Invalid --account-id. Expected UUID format.")
        }
    ...
    @discardableResult
    private func touchActiveGeminiAccount(provider: UsageProvider) async throws -> UUID? {
    ...
    private func classifyGeminiUsageIssue(errorCode: String?, errorMessage: String) -> GeminiUsageDoctorIssueCode {
        if let errorCode {
            switch errorCode {
            case "unsupported":
                return .unsupported
    ...
    private func diagnoseGeminiUsage(_ outcomes: [ProviderAccountUsageOutcome]) -> (healthy: Bool, issues: [String], hints: [String], diagnostics: [GeminiUsageDoctorIssue]) {
    ...
    private func formatGeminiAuthListText(
        provider: UsageProvider,
    ...
    private func formatGeminiAuthStatusText(
        provider: UsageProvider,
        accountCount: Int,
        activeAccountID: UUID?
    ) -> String {
    ...
    private func formatGeminiAuthMutationText(result: GeminiAuthMutationPayload) -> String {
        [
    ...
    private func formatGeminiUsageDoctorText(provider: UsageProvider, diagnosis: (healthy: Bool, issues: [String], hints: [String], diagnostics: [GeminiUsageDoctorIssue])) -> String {
        var lines: [String] = []
        lines.append("provider: \(provider.rawValue)")
...
private struct GeminiUsageEntry: Encodable, Sendable {
    let account: String
    let status: String
    let source: String?
...
private struct GeminiAuthAccountView: Encodable, Sendable {
    let id: String
    let label: String
    let method: String
    let isActive: Bool
...
private struct GeminiAuthListPayload: Encodable, Sendable {
    let provider: String
...
private struct GeminiAuthStatusPayload: Encodable, Sendable {
    let provider: String
...
private struct GeminiAuthMutationPayload: Encodable, Sendable {
    let provider: String
    let action: String
    let method: String?
...
private struct GeminiUsagePayload: Encodable, Sendable {
    let provider: String
    let refreshed: Bool
    let activeAccountID: String?
...
private struct GeminiUsageDoctorPayload: Encodable, Sendable {
    let provider: String
    let healthy: Bool
    let issues: [String]
    let hints: [String]
...
private struct GeminiUsageDoctorIssue: Encodable, Sendable {
    let account: String
    let code: GeminiUsageDoctorIssueCode
    let message: String
}

private enum GeminiUsageDoctorIssueCode: String, Encodable, Sendable {
    case binary
    case auth
    case parse
    case timeout
