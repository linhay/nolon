...
final class ResourceCenterViewModelTests: XCTestCase {
    ...
    func testBDD_GivenSelectedTab_WhenInitializeViewModel_ThenSelectionIsPreserved() async {
        let selectedTab = await MainActor.run { () -> ResourceContentTabType? in
    ...
    func testBDD_GivenGlobalSkillsRepository_WhenResolveEffectiveTargetProvider_ThenIgnoreSelectedProvider() async {
        let effectiveProvider = await MainActor.run { () -> Provider? in
            ...
            return viewModel.effectiveTargetProvider(
                for: .globalSkills,
                fallback: provider
            )
        }
    ...
    func testBDD_GivenGlobalSkillsRepository_WhenRefreshInstalledResources_ThenIgnoreProviderScopedInstallState() async throws {
        let fixture = try await MainActor.run { try TestFixture() }
        defer {
            Task { @MainActor in
                fixture.cleanup()
            }
        ...
        let repository = await MainActor.run { SkillRepository(nolonManager: fixture.nolonManager) }
        let globalSource = try await MainActor.run { try fixture.createSampleSkill(id: "global-only", name: "Global Only") }
        _ = try await MainActor.run { try repository.importSkill(from: globalSource) }

        let provider = await MainActor.run {
            makeProvider(id: "provider-a", name: "Provider A", root: fixture.tempRoot.path)
        }
        await MainActor.run {
        ...
        let installedSlugs = await MainActor.run { () -> Set<String> in
            let viewModel = ResourceCenterViewModel(selectedTab: .skills)
            ...
            return viewModel.installedSlugs
        }
