...
final class CodexAccountDisplaySectionsTests: XCTestCase {
    ...
    func testBDD_GivenHideErroredEnabled_WhenOutcomeIsFailure_ThenErroredAccountIsFilteredOut() {
        ...
        let sections = ProviderUsageEngine.makeCodexAccountDisplaySections(
            ...
            summaries: [
                healthy.id: CodexAuthSummary(cardKind: .chatgptAccount),
                failed.id: CodexAuthSummary(cardKind: .chatgptAccount)
            ],
            grouping: .none,
            sorting: .name,
            hideZeroQuotaAccounts: false,
    ...
    func testBDD_GivenHideErroredEnabled_WhenSummaryNeedsReloginButOutcomeSucceeded_ThenAccountIsHidden() {
        let relogin = CodexAuthAccount(id: UUID(), name: "Relogin", createdAt: .distantPast, relativeAuthPath: "auth/relogin.json")

        let sections = ProviderUsageEngine.makeCodexAccountDisplaySections(
            accounts: [relogin],
            outcomes: [Self.makeOutcome(account: relogin, label: "Relogin", remaining: 70)],
            summaries: [
                relogin.id: CodexAuthSummary(
                    cardKind: .chatgptAccount,
                    lastSyncFailedAt: Date(timeIntervalSince1970: 123),
                    lastSyncFailureMessage: "Authentication expired. Please log in again."
                )
            ],
            grouping: .none,
            sorting: .name,
    ...
    func testBDD_GivenConfigCardAuthFailure_WhenResolvingDisplayState_ThenIgnoresFailureState() {
        let viewModel = ProviderUsageEngine(provider: Self.makeCodexProvider())
    ...
    func testBDD_GivenOfficialAPIKeyFailure_WhenResolvingDisplayState_ThenIgnoresFailureState() {
        let viewModel = ProviderUsageEngine(provider: Self.makeCodexProvider())
        let outcome = ProviderAccountUsageOutcome(
            provider: .codex,
            account: .tokenAccount(.init(id: UUID(), label: "OpenAI", token: "", addedAt: 0, lastUsed: nil)),
            outcome: ProviderFetchOutcome(
                fetchKind: .cli,
                result: .failure(ProviderUsageError.authExpired(.codex))
            )
    ...
    func testBDD_GivenHideErroredEnabled_WhenOfficialAPIKeyFails_ThenAccountRemainsVisible() {
        let configured = CodexAuthAccount(id: UUID(), name: "Configured", createdAt: .distantPast, relativeAuthPath: "auth/configured.json")

        let sections = ProviderUsageEngine.makeCodexAccountDisplaySections(
            accounts: [configured],
            outcomes: [Self.makeFailureOutcome(account: configured, label: "Configured")],
            summaries: [configured.id: CodexAuthSummary(cardKind: .officialAPIKey)],
            grouping: .none,
            sorting: .name,
            hideZeroQuotaAccounts: false,
            hideErroredAccounts: true
        ...
        XCTAssertEqual(sections.count, 1)
        XCTAssertEqual(sections[0].items.map(\.displayName), ["Configured"])
    }
    ...
    func testBDD_GivenActiveCodexAccount_WhenTappingCard_ThenActivationIsSkipped() {
        ...
        let shouldActivate = viewModel.shouldActivateCodexAccountOnTap(id: account.id)
