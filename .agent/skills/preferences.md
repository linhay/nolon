...
- **UI 以截图/标注为准**：用户给出截图与具体 UI 调整要求时，视为权威约束（如卡片高度一致、表格展示、仅在宽度不足时显示 More），并在实现中严格对齐。
- **同类卡片跨 Tab 一次性统一**：当用户要求统一某类卡片视觉（如背景/边框/文字可读性）时，默认检查并同步 `skills/workflows/mcp` 等同级 tab，优先抽出复用组件，避免只修单处导致风格漂移。
- **深色模式可读性验收**：涉及卡片样式调整时，必须同时检查深色模式下“背景-边框-文字”的对比度；正文用 `Text.primary/secondary`，卡片背景优先 `DesignSystem.Colors.Background.elevated`，边框不低于可见阈值（如 `Component.border.opacity(0.6)`）。
- **提交前先做范围审查**：用户要求“提交”时，先 `git status` 审查未提交内容；默认只提交本轮明确确认的文件，避免把无关改动一并提交。
- **同语义按钮状态保持尺寸一致**：同一操作（如“激活/已激活”）在不同状态下必须保持字号、内边距、高度一致；只改颜色/禁用态，不改尺寸。
- **文件操作次要入口收敛到 overflow/context menu**：像“在 Finder 中显示”这类次要动作，优先放到右下角 `...` 菜单，并在卡片右键菜单提供同等入口。
- **Codex Sessions 先文档后实现**：涉及会话页的信息架构、分组、排序、缓存、刷新策略时，先对齐或补齐 `docs-linhay/features/`、`docs-linhay/dev/`、`docs-linhay/plans/`，再进入实现，不接受只改 UI 不补语义文档。
- **Codex Sessions 以稳定浏览优先**：大规模会话列表默认优先“缓存先展示、后台 reconcile、按 stale 刷新、排序稳定不跳动”，不要为了追求实时全量而破坏可滚动性和首屏稳定性。
...
- 用户要求“提交代码”时：先列出待提交文件，再按确认范围提交；若仍有未确认变更，保留在工作区不混入本次 commit。
- 用户指出“激活和已激活按钮尺寸变化”时：将“已激活”改为同样式禁用按钮而不是独立文本胶囊。
- 用户反馈会话页“每次进来都重扫 / 列表跳动 / 排序不稳”时：优先检查缓存命中、刷新触发条件、排序稳定性，而不是先加更多实时更新。
...
- 若本次改动涉及同类卡片样式，需在收尾说明已覆盖的 tab/组件清单（如 `SkillCardView`、`WorkflowCardView`、`McpServerCard`）。
- 若本次包含提交动作：收尾必须给出 commit hash、commit message、提交文件范围；并说明是否仍有未提交改动。
- 若本次涉及 `Codex Sessions`，需在收尾说明参考了哪些 `docs-linhay` 文档，以及是否遵守“文档先行 + 缓存优先 + 稳定浏览”的约束。
