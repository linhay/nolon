...
## 状态更新（2026-04-10）
1. Codex gateway 已从主工程彻底剥离，相关运行时代码、CLI 命令、UI 与依赖库均已移除。
2. 后续如需继续演进 gateway，目标形态为独立 Swift Package Manager 包，不再回放到当前主工程。
3. 本文档保留为历史 feature 记录；其中 gateway 相关验收、命令面与实现范围不再适用于当前仓库主线。
